...
try:
 ...
 if chain:
  ...
  bars=client._ib_get_historical_data_of_duration_and_barSize(contract,durationStr='1 Y',barSizeSetting='1 day')
  ...
  if bars is not None and not bars.empty:
   frame=client._raw_ib_data_to_df(bars);frame.to_parquet(DATA/'history.parquet');out['native_frame']={'rows':len(frame),'columns':list(frame.columns),'duplicates':bool(frame.index.has_duplicates),'monotonic':bool(frame.index.is_monotonic_increasing)}
